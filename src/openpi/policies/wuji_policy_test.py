...
def _example() -> dict:
    return {
        ...
        "actions": np.ones((32, 54), dtype=np.float32),
        "prompt": b"spray water",
    }


def test_wuji_inputs_map_three_cameras_and_preserve_54d_arrays():
    ...
    assert result["state"].shape == (54,)
    assert result["actions"].shape == (32, 54)
...
def test_wuji_inputs_reject_non_54d_state_and_actions():
    transform = wuji_policy.WujiInputs(model_type=_model.ModelType.PI05)

    data = _example()
    data["state"] = np.zeros((53,), dtype=np.float32)
    with pytest.raises(ValueError, match="state.*54"):
        transform(data)

    data = _example()
    data["actions"] = np.zeros((32, 53), dtype=np.float32)
    with pytest.raises(ValueError, match="actions.*54"):
        transform(data)


def test_wuji_outputs_return_54d_actions():
    actions = np.ones((16, 60), dtype=np.float32)

    result = wuji_policy.WujiOutputs()({"actions": actions})

    assert result["actions"].shape == (16, 54)


def test_wuji_norm_stats_inputs_validate_54d_without_images():
    result = wuji_policy.WujiNormStatsInputs()(
        {
            "state": np.zeros((54,), dtype=np.float32),
            "actions": np.zeros((32, 54), dtype=np.float32),
            "prompt": "ignored",
        }
    )
